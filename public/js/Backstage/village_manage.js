...
//查询（绘画表格）
    function into() {
        ...
        var ajax_url = '/Village/list'; // 定义数据请求路径 http://localhost:8080/JCSW/web/publicmenus/Village_List_ajax.action
        ...
        if ($p_id.find('#add_form').isValid()) {
            if (holdSubmit) {
                ...
                $.ajax({
                    ...
                    "timeout": 20000,
                    "async": false,
                    "url": sys_client+'/JCSW/web/publicmenus/Village_add_edit.action',
                    "data": addlist,
                    "jsonp":"callback",
                    "jsonpCaback":"handle",
    ...
    $p_id.find("#del_button").on("click",function () {
        ...
        if (holdSubmit) {
            ...
            $.ajax({
                ...
                "timeout": 20000,
                "async": false,
                "url": sys_client+'/JCSW/web/publicmenus/Village_delete_edit.action',
                "data": addlist,
                "jsonp":"callback",
                "jsonpCaback":"handle",
                "success": function (data) {
