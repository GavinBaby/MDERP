...
/*    function query_village($this,$change){
        var p_id_ = $this.val();
        if(!p_id_||p_id_==""){
            $change.empty();
            $change.append('<option value="">请先选择区域</option>');
        }else{
            $.ajax({
                "dataType": 'jsonp',
                "type": "get",
...
/*    $.ajax({
        ...
        "success": function (data) {
            ...
            $p_id.find("#areaid_s").on('change',function(){
                $p_id.find("#pid_s").empty();
                query_village($(this),$p_id.find("#pid_s"))
            });
            //小区联动（编辑）
            $p_id.find("#areaid").on('change',function(){
                $p_id.find("#p_id").empty();
                query_village($(this),$p_id.find("#p_id"))
            });
        },
        "error": function (data) {
            $p_id.find("#addAreaModal").modal('hide');
...
//查询（绘画表格）
    function into() {
        ...
        var ajax_url = '/Build/list'; // 定义数据请求路径 http://localhost:8080/JCSW/web/publicmenus/Village_List_ajax.action
        ...
        if ($p_id.find('#add_form').isValid()) {
            if (holdSubmit) {
                ...
                $.ajax({
                    ...
                    "timeout": 20000,
                    "async": false,
                    "url": sys_client+'/JCSW/web/publicmenus/Build_add_edit.action',
                    "data": addlist,
                    "jsonp":"callback",
                    "jsonpCaback":"handle",
                    "success": function (data) {
        ...
        if ($p_id.find('#add_form').isValid()) {
            if (holdSubmit) {
                ...
                $.ajax({
                    ...
                    "timeout": 20000,
                    "async": false,
                    "url": sys_client+'/JCSW/web/publicmenus/Build_update_edit.action',
                    "data": addlist,
                    "jsonp":"callback",
                    "jsonpCaback":"handle",
                    "success": function (data) {
    ...
    $p_id.find("#del_button").on("click",function () {
        ...
        if (holdSubmit) {
            ...
            $.ajax({
                ...
                "timeout": 20000,
                "async": false,
                "url": sys_client+'/JCSW/web/publicmenus/Build_delete_edit.action',
                "data": addlist,
                "jsonp":"callback",
                "jsonpCaback":"handle",
                "success": function (data) {
